...
対話しながら3フェーズを順番に進め、アウトプットの骨格を作ります。

| フェーズ              | やること                                   | 生成されるファイル   |
| --------------------- | ------------------------------------------ | -------------------- |
| Phase 1: ゴール       | 受け手に期待する行動・変化を決める         | `goal.md`            |
| Phase 2: 構成作成     | 素材・背景を集め、章・項に落とし込む       | `outline.md`         |
| Phase 3: レビュー     | 4観点で点検し深掘り・言語化                | `outline.md`（更新） |

各フェーズは自動で次に進みます。最初に「ゴールを先に決める」か「素材から先に考える」かを選べます。Phase 3（レビュー）は納得いくまで何度でも繰り返せます。

アウトラインは**制約フリー**で作成します。発表の場（LT、ブログなど）や制約条件（時間、文字数）、聴衆・読者はこの段階では考えず、話したい内容を自由に構成します。聴衆・読者の考慮や制約への適応は `/write` や `/talk` で行うため、同じアウトラインを異なる場や媒体で使い回せます。
...
| フェーズ          | やること                   | 生成されるファイル |
| ----------------- | -------------------------- | ------------------ |
| Phase 1: 執筆準備 | 想定読者・文体・トーン・方針を決める | `write-plan.md`    |
| Phase 2: 本文執筆 | セクション単位で下書き     | `draft.md`         |
| Phase 3: 推敲     | 5観点でレビューし仕上げ    | `article.md`       |
