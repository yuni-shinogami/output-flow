...
2. ディレクトリ名を `YYYYMMDD-NN_タイトル` 形式で自動生成する（CLAUDE.mdの命名規則に従う）。
   `output/<YYYYMMDD-NN_タイトル>/idea.md` に保存する。フォーマットは [templates/idea.md](templates/idea.md) を参照。
   ディレクトリが未作成なら作成する。
...
3. ディレクトリ名のプレフィックス `YYYYMMDD` 部分から作成日を抽出する。
4. 一覧を作成日の降順（新しい順）で表形式で表示する（CLAUDE.mdのソート順に従う）：
...
1. CLAUDE.mdの命名規則に従い `<name>` からディレクトリを解決し、`output/<project>/idea.md` を読み込み、現在の内容を提示する。
