...
- `docs/style-guide.md`（デザインシステム: スタイルガイド）
- `docs/patterns.md`（デザインシステム: レイアウトパターンカタログ）
- `docs/svg-guide.md`（SVG生成ガイド — slide-plan.mdにSVG計画がある場合に参照）

いずれか不在なら該当フェーズを案内（docs/ のファイルが不在でも続行可能だが、存在する場合は必ず参照すること）。
...
- **レイアウトクラス**: `img-text-right`, `img-text-left`, `img-center`, `img-full`, `img-grid` を使用（`docs/patterns.md` 参照）
- **SVGスタイル制約**: `docs/svg-guide.md` のカラーパレット・viewBox・品質ルールに従う
...
各スライドの執筆で意識すること:
- **全スライドに `<!-- _class: [パターン名] -->` を付与**する。slide-plan.mdのパターン選定テーブルに従い、`docs/patterns.md` のMarpコード例を参照する
- **1スライド1メッセージ**を原則とする
- **孤立文字の防止**: 大きいフォントのh1はパターンごとの1行文字数上限を意識し、超える場合は `<br>` で意味のまとまりごとに改行位置を制御する（上限目安: title=9文字, cover=14文字, emphasis=11文字, bridge=13文字, emphasis-detail=15文字, summary ol li=18文字。詳細は `docs/style-guide.md` の孤立文字防止セクション参照）
- `docs/style-guide.md` のカラーパレット・タイポグラフィ・NGパターンに従う
...
```bash
./scripts/take-screenshots.sh output/$ARGUMENTS/slide/slides.md output/$ARGUMENTS/slide/screenshots/
```
